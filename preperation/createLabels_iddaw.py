...
def process_folder(fn):
    ...
    # ds = fn.replace("gtFine", "level4Ids")
    dst = fn.replace("_mask.json", "_label{}s.png".format(args.id_type))
    ...
    if args.color:
        ...
        dst = fn.replace("_mask.json", "_labelColors.png")

        # do the conversion
        try:
            json2labelImg(fn, dst, 'color')
        except:
            tqdm.write("Failed to convert: {}".format(args.id_type))
            raise
